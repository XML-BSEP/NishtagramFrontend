...
import { ConfirmRegHistory } from 'src/app/model/user/confirmRegHistory';
...
export class AgentRegistrationComponent implements OnInit {
  ...
  register(){
    ...
    if(password===confirmPassword){
      ...
      this.registrationService.registerAgent(newUser).subscribe(
        success => {
          this.router.navigate(['/confirmRegistration'],
          {state:
            {data:
              new ConfirmRegHistory(username, email, "agent")
          }
